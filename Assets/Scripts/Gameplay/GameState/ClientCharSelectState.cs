...
namespace Unity.BossRoom.Gameplay.GameState
{
    ...
    public class ClientCharSelectState : GameStateBehaviour
    {
            ...
            else
            {
                if (seatIdx != -1)
                {
                    ...
                    if (isNewSeat)
                    {
                        ...
                        if (selectedCharacterGraphics == null)
                        {
                            m_ClassInfoBox.ConfigureForNoSelection();
                            return;
                        }
        ...
        {
            if (avatar == null || m_CharacterGraphicsParent == null)
            {
                Debug.LogError("ERO: invalid Avatar or character graphics parent in CharSelect.");
                return null;
            }

            if (!m_SpawnedCharacterGraphics.TryGetValue(avatar.Guid, out GameObject characterGraphics) || characterGraphics == null)
            {
                var prefab = EROAvatarReferenceRepairV509.GetCharacterSelectPrefab(avatar);
                if (prefab == null)
                {
                    Debug.LogError("ERO: no character-select prefab available for Avatar '" + avatar.name + "'.");
                    return null;
                }
                characterGraphics = Instantiate(prefab, m_CharacterGraphicsParent);
                m_SpawnedCharacterGraphics[avatar.Guid] = characterGraphics;
            }
